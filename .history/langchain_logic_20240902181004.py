...
from typing import Any, Dict, List  # Ensure these imports are here
...
# Initialize your queues and other global variables here
prompt_queue = Queue()
...
# Function to initialize the OpenAI LLM
def init_openai_llm():
    global llm
    try:
...
# Function to initialize the LLM and the retrieval chain
def init_llm():
    global llm, qa_chain
    try:
        db_path = os.path.join(os.getcwd(), 'db')
...
# Callback handler for the streaming response
...
# Initialize the VectorDB and LLM depending on whether OpenAI or local LLM is used
create_vectordb()
USE_OPENAI = True  # Ensure this matches your original file
if USE_OPENAI:
    init_openai_llm()
else:
